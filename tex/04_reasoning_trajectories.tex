...
We thus tried to identify global, defensible, utilty-neutral partitions in CoTs, which would have allowed us to split CoT streams into chunks, which could then have been classified as different types of reasoning operations.
...
These semantic boundaries are strongly visible in the hidden states. A question-disjoint latent detector reaches AUC 0.982 overall and 0.969 away from sentence endings. However, lexical TF--IDF alone reaches 0.892, showing that much of the type information is also present in the words themselves.
...
This control clarified what a reusable state must do: it must summarize the current history well enough for the task, \textbf{and} it must remain sufficient after repeated updates.
\FloatBarrier
...
As the 5-bit bar of \Cref{fig:rate-sufficiency} shows, capacity is not automatically better if the extra bits preserve irrelevant history. The padded code has the same size without this ambiguity and stays stable; the aliased code carries path information and loses closure.
