...
NAVY='#17324d'; TEAL='#1f8a8a'; ORANGE='#d97745'; GRAY='#7b8794'; LIGHT='#dbe4ea'; RED='#b55353'; GREEN='#4d8f62'
...
ax.errorbar(x,y,yerr=[y-lo,hi-y],fmt='o-',lw=2.4,ms=7,capsize=4,color=TEAL)
...
ax.set_xticks(idx); ax.set_xticklabels(corpora,rotation=18,ha='right')
...
for i,v in enumerate(rho): ax.text(v+.02,i,f'{v:.3f}',va='center',fontsize=12,fontweight='bold')
...
ax.legend(frameon=False,ncol=2,loc='lower left'); ax.grid(axis='y',alpha=.15); ax.spines[['top','right']].set_visible(False)
save(fig,'objective_auc.png')

# Exact finite control — reported horizon-32 values only
fig,ax=plt.subplots(figsize=(7.0,4.2))
labels=['Full history','12-bit suffix']
vals=[2.20,30.91]
ax.bar([0,1],vals,color=[TEAL,ORANGE],width=.58)
ax.set_xticks([0,1]); ax.set_xticklabels(labels)
ax.set_ylabel('closed-loop regret at horizon 32')
ax.set_ylim(0,34)
for i,v in enumerate(vals): ax.text(i,v+1.0,f'{v:.2f}',ha='center',fontweight='bold',fontsize=14)
ax.grid(axis='y',alpha=.15); ax.spines[['top','right']].set_visible(False)
save(fig,'snapshot_regret.png')
...
for i,v in enumerate(vals): ax.text(i,v+2.5,f'{v:.1f}%',ha='center',fontweight='bold')
ax.grid(axis='y',alpha=.15); ax.spines[['top','right']].set_visible(False)
save(fig,'native_handoff.png')
...
for i,v in enumerate(vals): ax.text(i,v+2.4,f'{v:g}%',ha='center',fontweight='bold')
ax.grid(axis='y',alpha=.15); ax.spines[['top','right']].set_visible(False)
save(fig,'local_global.png')

# Bridge experiment — rate thresholds
labels=['Reasoning','Answer']
m=[1.070,.578]; q=[.471,.575]
idx=np.arange(2); w=.34
fig,ax=plt.subplots(figsize=(6.4,4.1))
ax.bar(idx-w/2,m,w,label='Mistral-7B',color=NAVY)
ax.bar(idx+w/2,q,w,label='Qwen2.5-7B',color=TEAL)
ax.set_xticks(idx); ax.set_xticklabels(labels); ax.set_ylabel(r'$R^*(0.90)$  [bits / value]')
ax.legend(frameon=False); ax.grid(axis='y',alpha=.15); ax.spines[['top','right']].set_visible(False)
...
full=[.567,.101]; reason=[.568,.100]
idx=np.arange(2); w=.34
fig,ax=plt.subplots(figsize=(6.5,4.1))
ax.bar(idx-w/2,full,w,label='Full response',color=NAVY)
ax.bar(idx+w/2,reason,w,label='Reasoning only',color=TEAL)
