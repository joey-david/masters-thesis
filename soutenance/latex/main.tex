...
\framesubtitle{For Part I, the key object is the input--output function, not the internal circuit decomposition}
\begin{center}
\begin{tikzpicture}[node distance=.28cm]
  \node[boxstyle,fill=Pale,minimum width=1.45cm,minimum height=.68cm] (tok) {tokens};
  \node[boxstyle,right=of tok,minimum width=1.65cm,minimum height=.68cm] (emb) {embeddings};
  \node[boxstyle,right=of emb,minimum width=2.65cm,minimum height=.68cm] (tr) {Transformer blocks};
  \node[boxstyle,right=of tr,minimum width=1.25cm,minimum height=.68cm] (log) {logits};
  \node[tealbox,right=of log,minimum width=2.55cm,minimum height=.68cm] (p) {$p_\theta(y_t\mid x,y_{<t})$};
  \draw[arr] (tok)--(emb); \draw[arr] (emb)--(tr); \draw[arr] (tr)--(log); \draw[arr] (log)--(p);
  \node[below=.48cm of tr,align=center,text width=4.8cm,font=\small] {self-attention + MLP + residual stream\\repeated through many layers};
  \draw[decorate,decoration={brace,amplitude=4pt},draw=Muted] ([xshift=-1.05cm,yshift=-.28cm]tr.south) -- ([xshift=1.05cm,yshift=-.28cm]tr.south);
\end{tikzpicture}
\end{center}
\vspace{0.15cm}
\begin{columns}[T,onlytextwidth]
\column{0.55\textwidth}
{\color{Teal}\large Autoregressive objective}\par\vspace{0.08cm}
\[
  p_\theta(y_{1:T}\mid x)=\prod_{t=1}^T p_\theta(y_t\mid x,y_{<t})
\]
\vspace{-0.18cm}{\small\color{Muted}prefix $\longrightarrow$ next-token distribution}\par\vspace{0.12cm}{\scriptsize\color{Muted}\parencite{vaswani2017attention}}
\column{0.41\textwidth}
{\color{Teal}\large Part I abstraction}\par\vspace{0.08cm}
\[
  f_{\theta_0}: x \mapsto p(y\mid x)
\]
\vspace{-0.22cm}
\begin{itemize}\small\setlength\itemsep{1.5pt}
  \item pretrained parameters $\theta_0$
  \item large differentiable function
  \item adaptation = parameter perturbation
\end{itemize}
\end{columns}
\note{\small\begin{itemize}
  \item Une LLM moderne reste, au niveau utile ici, un grand réseau autoregressif qui transforme un préfixe en distribution du prochain token.
  \item L'attention et les MLPs sont bien réels et importants, mais ils ne changent pas la définition de notre mesure.
  \item Pour la Partie I, on peut donc noter le modèle comme une fonction $f_{\theta_0}$ et étudier la perturbation de paramètres introduite par l'apprentissage.
  \item Citation de fond: Transformer de Vaswani et al., déjà dans le rapport.
\end{itemize}}
\end{frame}
...
\begin{tikzpicture}[>=Latex]
  \node (x) at (0,0) {$x$};
  \coordinate (split) at (1.0,0); \fill (split) circle (1.3pt); \draw[arr] (x)--(split);
  \node[draw=Teal!70!black,fill=Pale,minimum width=3.2cm,minimum height=.95cm,right=1.1cm of split] (W) {\Large $W$ \;\small frozen};
  \node[draw=Amber!80!black,fill=Sand,minimum width=1.1cm,minimum height=.8cm,below=.55cm of W.west,anchor=west] (A) {$A$};
  \node[draw=Amber!80!black,fill=Sand,minimum width=1.1cm,minimum height=.8cm,right=.5cm of A] (B) {$B$};
  \node[draw,circle,inner sep=1.5pt,right=1.0cm of W] (sum) {$+$};
  \node[right=.75cm of sum] (h) {$h$};
  \draw[arr] (split) |- (W); \draw[arr] (W)--(sum);
  \draw[arr] (split) |- (A); \draw[arr] (A)--node[above,font=\scriptsize]{$r\ll d$}(B); \draw[arr] (B) -| (sum);
  \draw[arr] (sum)--(h);
  \node[below=.25cm of A,font=\scriptsize,text=Muted] {trainable};
\end{tikzpicture}
...
\small
\begin{itemize}
  \item $W$ is shared and frozen
  \item $A,B$ are trained
  \item trainable values: $r(d_{in}+d_{out})$ instead of $d_{in}d_{out}$
  \item same rank $\neq$ same code length
\end{itemize}
\tinycite{hu2022lora}
\end{columns}
...
\note{\small\begin{itemize}
  \item LoRA gèle $W$ et apprend une factorisation de rang faible $BA$.
  \item Cela réduit énormément le nombre de valeurs entraînables.
  \item Mais le rang ne donne pas un nombre de bits: les valeurs numériques doivent encore être représentées.
  \item C'est la première distinction scientifique importante de la Partie I: dimension paramétrique et information utile ne sont pas la même chose.
\end{itemize}}
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}{QLoRA: quantize the receiver, train the adapter}
\framesubtitle{Memory-efficient training leaves the conceptual channel unchanged}
\begin{tikzpicture}[node distance=.45cm]
  \node[amberbox,minimum width=2.2cm,minimum height=.85cm,align=center] (x) {minibatch};
  \node[tealbox,right=.65cm of x,minimum width=3.0cm,minimum height=1.15cm,align=center] (q) {\bfseries 4-bit backbone\\\small NF4 weights, frozen};
  \node[boxstyle,right=.65cm of q,minimum width=2.4cm,minimum height=1.15cm,align=center] (dq) {\bfseries dequantize\\\small for compute};
  \node[amberbox,right=.65cm of dq,minimum width=2.4cm,minimum height=1.15cm,align=center] (lora) {\bfseries LoRA $A,B$\\\small trainable};
  \draw[arr] (x)--(q); \draw[arr] (q)--(dq); \draw[arr] (dq)--(lora);
  \draw[-{Latex[length=2mm]},draw=Amber,line width=1pt] (lora.south) -- ++(0,-.75) -| node[pos=.25,below,font=\scriptsize,text=Amber]{gradients update adapter only} ([xshift=0.0cm]lora.south);
\end{tikzpicture}
\vspace{0.55cm}
\begin{columns}[T,onlytextwidth]
\column{0.48\textwidth}
\textbf{QLoRA helps us train}
\begin{itemize}\small
  \item lower backbone memory
  \item higher-precision adapter optimization
  \item frozen quantized base model
\end{itemize}
\column{0.48\textwidth}
\textbf{Our rate question is different}
\begin{itemize}\small
  \item after training, fix one adapter
  \item compress only its representation
  \item reload and score behavior
\end{itemize}
\end{columns}
\vfill\tinycite{dettmers2023qlora}
\note{\small\begin{itemize}
  \item QLoRA garde le backbone pré-entraîné en représentation 4 bits et entraîne des adapters LoRA à travers lui.
  \item C'est d'abord une technique d'efficacité mémoire à l'entraînement.
  \item Notre question de compression vient après: on fixe un adapter appris et on mesure le nombre de bits nécessaires pour le stocker sans perdre le comportement.
  \item Donc QLoRA et notre protocole parlent tous deux de quantification, mais avec des objectifs distincts.
\end{itemize}}
\end{frame}
